"""
add_car_view.py — Адмін: додавання авто (різні типи полів: dropdown, radio, slider, date picker, switch).
"""
...
_COLORS = [
    ("White", "Білий", ft.Colors.WHITE),
    ("Black", "Чорний", ft.Colors.BLACK),
    ("Silver", "Срібний", ft.Colors.BLUE_GREY_300),
    ("Gray", "Сірий", ft.Colors.GREY_600),
    ("Blue", "Синій", ft.Colors.BLUE_700),
    ("Red", "Червоний", ft.Colors.RED_700),
    ("Green", "Зелений", ft.Colors.GREEN_700),
]
...
def _section(title: str, *controls) -> ft.Column:
    return ft.Column(
        controls=[
            ft.Text(title, size=12, weight=ft.FontWeight.W_600, color=ft.Colors.BLUE_GREY_600),
            *controls,
        ],
        spacing=8,
    )
...
def _labeled_slider(label: str, slider: ft.Slider, value_text: ft.Text) -> ft.Column:
    return ft.Column(
        controls=[
            ft.Row(
                controls=[
                    ft.Text(label, size=13, color=ft.Colors.BLUE_GREY_800),
                    value_text,
                ],
...
class AddCarView:
    ...
    def build(self) -> ft.View:
        if not AppState.is_admin():
            return ft.View(
                route="/admin/add-car",
                controls=[ft.Text("Доступ заборонено", size=22, color=ft.Colors.RED_600)],
            )
        ...
        brand_dd = ft.Dropdown(
            label="Марка *",
            options=[ft.DropdownOption(key=b, text=b) for b in _BRANDS],
            value="Toyota",
            border_radius=10,
            border_color=ft.Colors.BLUE_GREY_200,
            prefix_icon=ft.Icons.DIRECTIONS_CAR,
        )
        brand_custom = ft.TextField(
            label="Вкажіть марку",
            visible=False,
            border_radius=10,
            border_color=ft.Colors.BLUE_GREY_200,
            hint_text="наприклад Lexus",
        )
        ...
        model = ft.TextField(
            label="Модель *",
            prefix_icon=ft.Icons.CAR_RENTAL,
            hint_text="наприклад Camry",
            border_radius=10,
            border_color=ft.Colors.BLUE_GREY_200,
            focused_border_color=ft.Colors.DEEP_PURPLE_700,
        )

        year_display = ft.Text("2023", size=18, weight=ft.FontWeight.W_700, color=ft.Colors.DEEP_PURPLE_800)
        year_picker = ft.DatePicker(
            ...
            help_text="Рік випуску авто",
            confirm_text="OK",
            cancel_text="Скасувати",
        )

        def on_year_picked(e):
            if year_picker.value:
                year_display.value = str(year_picker.value.year)
                self._page.update()

        year_picker.on_change = on_year_picked

        year_row = ft.Container(
            content=ft.Row(
                controls=[
                    ft.OutlinedButton(
                        content=ft.Row(
                            controls=[
                                ...
                                ft.Text("Обрати рік", size=13),
                            ],
                            spacing=8,
                            tight=True,
                        ),
                        on_click=lambda _: self._page.show_dialog(year_picker),
                    ),
                    year_display,
                ],
                spacing=16,
        ...
        price_slider = ft.Slider(
            ...
            value=50,
            label="{value}",
            on_change=lambda e: self._update_slider_label(price_value, f"${int(e.control.value)} / день"),
        )

        seats_value = ft.Text("5 місць", size=13, weight=ft.FontWeight.W_600, color=ft.Colors.DEEP_PURPLE_700)
        seats_slider = ft.Slider(
            min=2,
            max=9,
            divisions=7,
            value=5,
            label="{value}",
            on_change=lambda e: self._update_slider_label(seats_value, f"{int(e.control.value)} місць"),
        )

        mileage_value = ft.Text("15 000 км", size=13, weight=ft.FontWeight.W_600, color=ft.Colors.DEEP_PURPLE_700)
        mileage_slider = ft.Slider(
            min=0,
            max=200000,
            divisions=40,
            value=15000,
            label="{value}",
            on_change=lambda e: self._update_slider_label(
                mileage_value, f"{int(e.control.value):,} км".replace(",", " ")
            ),
        )
        ...
        plate = ft.TextField(
            ...
            hint_text="AA 1234 KK",
            border_radius=10,
            border_color=ft.Colors.BLUE_GREY_200,
            text_capitalization=ft.TextCapitalization.CHARACTERS,
        )

        color_rg = ft.RadioGroup(
            value="White",
            content=ft.Wrap(
                spacing=10,
                run_spacing=8,
                controls=[
                    ft.Row(
                        controls=[
                            ft.Radio(value=key, label=label),
                            ft.Container(
                                width=18,
                                height=18,
                                bgcolor=swatch,
                    ...
                    for key, label, swatch in _COLORS
                ],
            ),
        )

        transmission_rg = ft.RadioGroup(
            value="Automatic",
            content=ft.Row(
                controls=[
                    ft.Radio(value="Automatic", label="Автомат"),
                    ft.Radio(value="Manual", label="Механіка"),
                ],
                spacing=24,
            ),
        )

        fuel_rg = ft.RadioGroup(
            value="Gasoline",
            content=ft.Column(
                controls=[
                    ft.Row(
                        controls=[
                            ft.Radio(value="Gasoline", label="Бензин"),
                            ft.Radio(value="Diesel", label="Дизель"),
                        ],
                        spacing=24,
                    ),
                    ft.Row(
                        controls=[
                            ft.Radio(value="Hybrid", label="Гібрид"),
                            ft.Radio(value="Electric", label="Електро"),
                        ],
                        spacing=24,
                    ),
                ],
        ...
        async def pick_photos(_):
            try:
                files = await file_picker.pick_files(
                    dialog_title="Оберіть фото авто",
            ...
            for f in files:
                ...
                self._photo_previews.controls.append(
                    ft.Container(
                        content=ft.Image(
                            src=prev,
                            width=70,
        ...
        pick_btn = ft.Button(
            content=ft.Row(
                controls=[
                    ft.Icon(ft.Icons.ADD_PHOTO_ALTERNATE, color=ft.Colors.WHITE, size=18),
                    ft.Text("Додати фото", color=ft.Colors.WHITE, size=13),
                ],
                spacing=8,
            ),
        ...
        async def save_car(_):
            ...
            try:
                car_data = {
                    ...
                    "price": p,
                    "seats": seats_v,
                    "transmission": transmission_rg.value,
                    "fuel": fuel_rg.value,
                    "plate": plate.value.strip().upper(),
                    "color": color_rg.value or "White",
                    "mileage": mileage_v,
                    "description": desc,
                    "photos": [],
                    "added_by": AppState.current_user["email"],
                    "is_premium": bool(premium_switch.value),
                }
        ...
        save_btn = ft.Button(
            content=ft.Text("Зберегти", size=15, weight=ft.FontWeight.W_600, color=ft.Colors.WHITE),
            style=ft.ButtonStyle(
        ...
        form_card = ft.Container(
            content=ft.Column(
                controls=[
                    ft.Text("Дані авто", size=16, weight=ft.FontWeight.W_600, color=ft.Colors.BLUE_GREY_900),
                    ft.Divider(height=12, color=ft.Colors.BLUE_GREY_100),
                    _section("Основне", brand_dd, brand_custom, model),
                    _section("Рік випуску *", year_row),
                    _section(
                        "Ціна та місткість",
                        _labeled_slider("Ціна ($/день) *", price_slider, price_value),
                        _labeled_slider("Кількість місць", seats_slider, seats_value),
                    ),
                    _section("Номер", plate),
                    _section("Колір", color_rg),
                    _section("Коробка передач", transmission_rg),
                    _section("Тип палива", fuel_rg),
                    _section("Пробіг", _labeled_slider("Пробіг (км)", mileage_slider, mileage_value)),
                    _section(
                        "Додатково",
                        premium_switch,
                        ft.Row(controls=[insured_cb, ac_cb], spacing=24, wrap=True),
                    ),
                    _section("Опис", description),
                    ft.Divider(height=12, color=ft.Colors.BLUE_GREY_100),
                    ft.Text("Фотографії", size=14, weight=ft.FontWeight.W_600, color=ft.Colors.BLUE_GREY_900),
                    ft.Row(
                        controls=[pick_btn, self._photo_status_text],
                        spacing=12,
                        vertical_alignment=ft.CrossAxisAlignment.CENTER,
                    ),
                    self._photo_previews,
                    ft.Divider(height=12, color=ft.Colors.BLUE_GREY_100),
                    error_text,
        ...
        return ft.View(
            route="/admin/add-car",
            padding=0,
            bgcolor=ft.Colors.with_opacity(0.07, ft.Colors.DEEP_PURPLE_900),
            services=[file_picker, year_picker],
            controls=[
                ft.Column(
                    spacing=0,
                    expand=True,
    ...
    @staticmethod
    def _update_slider_label(text: ft.Text, value: str):
        text.value = value
        text.update()
