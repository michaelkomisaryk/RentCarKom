"""
sort_filter_bar.py — Sort/filter UI: dropdown, segmented controls, active chip states.
"""
...
class SortFilterBar:
    """Builds filter/sort controls and reports changes via callbacks."""
    ...
    def _rebuild_fuel_chips(self):
        ...
        self._fuel_chips_row.controls = [
            _chip(
                label,
                active=self.fuel == key,
                icon=icon,
                on_click=lambda e, k=key: self._set_fuel(k),
            )
    ...
    def build(
        ...
        *,
        ...
        def on_sort_seg(e: ft.ControlEvent):
            ...
            if sel:
                self.sort_field = sel[0]  # type: ignore[assignment]
                self._update_sort_ui()
                self._notify()
        ...
        def on_sort_dd(e: ft.ControlEvent):
            self._set_sort((e.control.value or "price"))  # type: ignore[arg-type]

        sort_dd = ft.Dropdown(
            label="Сортувати за",
            width=200,
            value=self.sort_field,
            options=[ft.DropdownOption(key=f, text=SORT_LABELS[f]) for f in _SORT_FIELDS],
            border_radius=12,
            text_size=13,
                ...
                controls=[
                    ft.Row(
                        controls=[
                            ...
                            ft.Text("Фільтри та сортування", size=17, weight=ft.FontWeight.W_700, color=ft.Colors.BLUE_GREY_900),
                    ...
                    ft.ResponsiveRow(
                        ...
                        run_spacing=12,
                        controls=[
                            ft.Container(col={"xs": 12, "md": 5}, content=sort_dd),
                            ft.Container(
                                col={"xs": 12, "md": 7},
                                content=ft.Row(
                                    controls=[self._sort_seg, self._dir_btn],
                                    spacing=8,
                                    vertical_alignment=ft.CrossAxisAlignment.CENTER,
