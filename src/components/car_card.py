"""
car_card.py — Premium glass/neon car card (stable hover, no layout jitter).
"""
...
def build_car_card(page: ft.Page, car: dict) -> ft.Container:
    ...
    if src0:
        ...
        photo = ft.Container(
            ...
            gradient=ft.LinearGradient(
                ...
                colors=[
                    ft.Colors.with_opacity(0.35, accent),
                    ft.Colors.with_opacity(0.15, ft.Colors.DEEP_PURPLE_900),
                ],
            ),
            content=ft.Icon(
    ...
    photo_stack = ft.Stack(
        ...
        controls=[
            photo,
            ft.Container(content=badges_wrap, top=8, left=8, right=8),
        ],
    )
    ...
    base_info = ft.Column(
        controls=[
            ...
            ft.Row(
                controls=[
                    ft.Text(
                        f"${float(car.get('price', 0)):.0f}",
                        size=20,
                        weight=ft.FontWeight.W_700,
                        color=ft.Colors.CYAN_ACCENT_200,
                    ),
                    ft.Text("/день", size=12, color=ft.Colors.with_opacity(0.54, ft.Colors.WHITE)),
                    ft.Container(expand=True),
                    ft.Row(
                        controls=[
                            ft.Icon(ft.Icons.STAR, size=14, color=ft.Colors.AMBER_300),
                            ft.Text(f"{rating:.1f}", size=12, color=ft.Colors.with_opacity(0.7, ft.Colors.WHITE)),
                        ],
                        spacing=2,
                    ),
                ],
    ...
    hover_overlay = ft.Container(
        opacity=0,
        animate_opacity=_ANIM,
        bgcolor=ft.Colors.with_opacity(0.88, ft.Colors.BLUE_GREY_900),
        border_radius=ft.BorderRadius(0, 0, 12, 12),
        padding=ft.Padding(14, 10, 14, 12),
        content=ft.Column(
            controls=[
                ft.Row(
                    controls=[
                        ft.Row(
                            controls=[
                                ft.Icon(ft.Icons.STAR, size=16, color=ft.Colors.AMBER_300),
                                ft.Text(f"{rating:.1f}", size=13, weight=ft.FontWeight.BOLD, color=ft.Colors.WHITE),
                                ft.Text(
                                    f"· {popularity} оренд",
                                    size=11,
                        ...
                        ft.Text(
                            f"${float(car.get('price', 0)):.0f}/день",
                            size=15,
                            weight=ft.FontWeight.W_700,
                ...
                ft.Row(
                    controls=[
                        ft.Icon(ft.Icons.EVENT_SEAT, size=14, color=ft.Colors.CYAN_100),
                        ft.Text(
                            f"{car.get('seats', 5)} місць",
                            size=11,
                            color=ft.Colors.with_opacity(0.7, ft.Colors.WHITE),
                        ),
                        ft.Icon(ft.Icons.SPEED, size=14, color=ft.Colors.CYAN_100),
                        ft.Text(
                            f"{int(car.get('mileage', 0)):,} км".replace(",", " "),
                            size=11,
                            color=ft.Colors.with_opacity(0.7, ft.Colors.WHITE),
                        ),
                    ],
                ...
                ft.Row(
                    controls=[
                        ft.IconButton(
                            icon=ft.Icons.FAVORITE if AppState.is_favorite(car_id) else ft.Icons.FAVORITE_BORDER,
                            icon_color=ft.Colors.PINK_ACCENT_200
                            if AppState.is_favorite(car_id)
                            else ft.Colors.with_opacity(0.7, ft.Colors.WHITE),
                            icon_size=20,
                            tooltip="Обране",
                            style=ft.ButtonStyle(
                                bgcolor=ft.Colors.with_opacity(0.2, ft.Colors.PINK_200),
                                shape=ft.CircleBorder(),
                            ),
                            on_click=on_favorite,
                        ),
                        ft.OutlinedButton(
                            content=ft.Text("Деталі", size=12),
                            style=ft.ButtonStyle(
                                color=ft.Colors.WHITE,
                                side=ft.BorderSide(1, ft.Colors.with_opacity(0.5, accent)),
                            ),
                            on_click=go_to_detail,
                        ),
                        ft.Button(
                            content=ft.Text("Орендувати", size=12, weight=ft.FontWeight.W_600),
                            style=ft.ButtonStyle(
    ...
    glass_body = ft.Container(
        content=ft.Column(controls=[photo_stack, details_stack], spacing=0, tight=True),
        bgcolor=ft.Colors.with_opacity(0.72, ft.Colors.BLUE_GREY_900),
    ...
    sweep = ft.Container(
        ...
        gradient=ft.LinearGradient(
            ...
            colors=[
                ...
                ft.Colors.with_opacity(0.75, accent),
                ft.Colors.with_opacity(0.75, hover_accent),
                ft.Colors.TRANSPARENT,
            ],
        ),
    )

    border_ring = ft.Container(
        border_radius=16,
        padding=2,
        gradient=ft.LinearGradient(
            begin=ft.Alignment(-1, -1),
            ...
            colors=[
                ft.Colors.with_opacity(0.4, accent),
                ft.Colors.with_opacity(0.18, ft.Colors.DEEP_PURPLE_400),
                ft.Colors.with_opacity(0.35, hover_accent),
            ],
        ),
        content=ft.Column(controls=[sweep, glass_body], spacing=0, tight=True),
    )
    ...
    visual = ft.Container(
        ...
        scale=ft.Scale(1.0),
        animate_scale=_ANIM,
        shadow=neon_card_glow(accent, strong=False),
    )
    ...
    def on_hover(e: ControlEvent):
        ...
        glass_body.border = ft.Border.all(
            2 if h else 1,
            ft.Colors.with_opacity(0.85 if h else 0.28, hover_accent if h else accent),
        )
