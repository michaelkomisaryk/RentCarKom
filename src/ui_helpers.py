"""
ui_helpers.py — SnackBar, image paths, safe error messages for rentCarKom.
"""
...
def show_message(page: ft.Page, msg: str, *, error: bool = False) -> None:
    """Non-blocking toast; avoids crashing if dialog stack is busy."""
    try:
        page.show_dialog(
            ft.SnackBar(
...
def image_src_for_flet(stored: str | None) -> str | None:
    """
    Flet Image src must be URL, path relative to assets_dir (default 'assets'),
    base64, or bytes. DB stores 'cars/<car_id>/<file>' or legacy absolute paths.
    """
    if not stored:
        return None
...
def image_src_for_preview(path: str | None, file_bytes: bytes | None, filename: str) -> str | bytes:
    """Preview in form: use bytes (web) or filesystem path (desktop)."""
...
def user_safe_error(exc: BaseException, default: str = "Something went wrong. Check your input.") -> str:
    """Map common errors to Ukrainian-friendly messages."""
