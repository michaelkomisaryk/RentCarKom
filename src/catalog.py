...
SORT_LABELS :dict [str ,str ]={'price':'Ціна','popularity':'Популярність','newest':'Новинки','rating':'Рейтинг','availability':'Доступність'}
...
def filter_cars (query :CatalogQuery )->list [dict ]:
    ...
    if q :
        cars =[c for c in cars if q in c .get ('brand','').lower ()or q in c .get ('model','').lower ()or q in c .get ('plate','').lower ()]
    return cars
...
def paginate_cars (cars :list [dict ],page :int ,page_size :int )->CatalogResult :
    ...
    return CatalogResult (items =cars [start :end ],total =total ,page =page ,page_size =page_size ,total_pages =total_pages )
